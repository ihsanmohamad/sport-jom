  ...
  setup() {
    ...
    const fetchSearchResults = async (searchTerm) => {
      ...
      if(searchTerm) {
        ...
        state.searchResults = [];
        const res = await axios.get(`http://192.168.1.5:8000/facility/filter?category=${category}&name=${searchTerm}`);
    ...
    const fetchResults = async () => {
      state.loading = true;
        ...
        const res = await axios.get(`http://192.168.1.5:8000/facility/filter?category=${category}`);
    ...
    // const fetchDrinksByIngredient = async (category) => {
    //   state.loading = true;

    //   const res = await axios.get(`https://www.thecocktaildb.com/api/json/v1/1/filter.php?i=test`);
    ...
    // fetchDrinksByIngredient(category);
    return {
      router,
      state,
      category,
